...
namespace EditorControl.Model
{
    ...
    internal static class ShapeHelper
    {
        ...
        public static float GetInertia(IEditorShape shape)
        {
            ...
            float density = p.MassType1 == PhysicEngine.ExportData.RigidBody.MassData.MassType.Density ? p.Density : p.Mass / GetArea(shape);
        ...
        {
            List<KeyValuePair<float, IEditorShape>> list = new List<KeyValuePair<float, IEditorShape>>();
            foreach (var shape in shapes)
            {
                ...
                {
                    list.Add(new KeyValuePair<float, IEditorShape>(shape.GetArea(), shape));
                }
            }
