...
using PhysicEngine.UnitTests.CollisionResolution;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using System.Threading.Tasks;
using static PhysicEngine.UnitTests.CollisionResolution.NoGravityTestHelper;
using static PhysicEngine.UnitTests.TestHelper.SimulateSeveralTimeSteps;
    ...
    internal class SimulateSeveralTimeSteps
    {
        public class Body
        {
            ...
            public Body(IRigidBody body)
            {
                ...
                Velocity = MathHelp.Round(body.Velocity, roundDecimalPlaces);
                AngularVelocity = MathHelp.Round(body.AngularVelocity, roundDecimalPlaces);
                Position = new Vector2D(body.Center);
                Angle = body.Angle;
            }
        ...
        public class Joint
        {
            public Vector2D Anchor1;
            public Vector2D Anchor2;

            public Joint(IJoint joint)
            {
                int roundDecimalPlaces = 7;  //Runde 7 Stellen nach dem Komma damit der Float-Vergleich im UnitTest klappt

                this.Anchor1 = MathHelp.Round(joint.Anchor1, roundDecimalPlaces);
                this.Anchor2 = MathHelp.Round(joint.Anchor2, roundDecimalPlaces);
            }
        }

        public class MouseData
        {
            public Vector2D Position; //Position der Maus
        ...
        public class ExtraSettings
        {
            ...
            public PhysicScene.Helper? ResolverHelper = null;
        }
        ...
        public class MouseEvent
        {
            ...
            public MouseEvent(EventType mouseType, Vector2D position, int time)
            {
        ...
        public static SimulateSeveralTimeSteps DoTest(string sceneFilePath, float timeStepTickRate, bool useGlobalSolver, int pgsIterations = 100, int timeSteps = 50, ExtraSettings extraSettings = null)
        {
            ...
            if (extraSettings != null)
            {
                ...
                scene.HasGravity = extraSettings.HasGravity;
                if (extraSettings.Solver != null) scene.Solver = (PhysicScene.SolverType)extraSettings.Solver;
                if (extraSettings.ResolverHelper != null) scene.ResolverHelper = (PhysicScene.Helper)extraSettings.ResolverHelper;
            }
        ...
        class MouseSimulationData
        {
            ...
            public Vector2D MousePosition = null;
        ...
        public static float[] GetAnchorDistancesFromJoint(TimeStep[] timeSteps, int jointIndex)
        {
            return timeSteps.Select(x => (x.Joints[jointIndex].Anchor1 - x.Joints[jointIndex].Anchor2).Length()).ToArray();
        }
