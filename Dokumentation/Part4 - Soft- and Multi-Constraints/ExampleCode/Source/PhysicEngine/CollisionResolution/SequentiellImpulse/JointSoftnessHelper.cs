...
namespace PhysicEngine.CollisionResolution.SequentiellImpulse
{
    static internal class JointSoftnessHelper
    {
        ...
        public static void LinearFrequencyToStiffness(float frequencyHertz, float dampingRatio, IRigidBody bodyA, IRigidBody bodyB, out float stiffness, out float damping)
        {
        ...
        public static void LinearStiffnessToFrequency(float stiffness, float damping, IRigidBody bodyA, IRigidBody bodyB, out float frequencyHertz, out float dampingRatio)
        {
            float massA = (bodyA != null && bodyA.InverseMass != 0) ? 1.0f / bodyA.InverseMass : 0;
            float massB = (bodyB != null && bodyB.InverseMass != 0) ? 1.0f / bodyB.InverseMass : 0;
            float mass;
