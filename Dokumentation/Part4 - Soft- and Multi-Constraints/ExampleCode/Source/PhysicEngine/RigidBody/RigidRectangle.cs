...
namespace PhysicEngine.RigidBody
{
    public class RigidRectangle : IRigidBody, ICollidableRectangle
    {
        ...
        public RigidRectangle(Vector2D center, Vector2D size, float angle, MassData massData)
        {
            ...
            this.vertexLocal = new Vector2D[]
            {
                new Vector2D(-size.X / 2, -size.Y / 2), //TopLeft
                new Vector2D(+size.X / 2, -size.Y / 2), //TopRight
                new Vector2D(+size.X / 2, +size.Y / 2), //BottomRight
                new Vector2D(-size.X / 2, +size.Y / 2), //BottomLeft
            };

            this.Vertex = new Vector2D[this.vertexLocal.Length];
        ...
        public void Rotate(float angle)
        {
            ...
            Matrix2x2 rot = Matrix2x2.Rotate(this.Angle);
            for (int i = 0; i < this.vertexLocal.Length; i++)
            {
                this.Vertex[i] = this.Center + rot * this.vertexLocal[i];
            }
