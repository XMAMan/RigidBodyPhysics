...
namespace PhysicEngine.CollisionResolution.EnterTheMatrix
{
    ...
    internal class EquationOfMotionData
    {
        ...
        public EquationOfMotionData(List<IRigidBody> bodies, RigidBodyCollision[] collisions, float dt, SolverSettings settings)
        {
            ...
            this.initialLambda = Matrix.GetColumVectorWithZeros(this.minLambda.RowCount);
        ...
        public void SetVelocityValues(List<IRigidBody> bodies)
        {
            ...
            for (int i = 0; i < bodies.Count; i++)
            {
                ...
                bodies[i].Velocity.X = MathHelp.Round(vNew[i * 3 + 0], roundDecimalPlaces);
                bodies[i].Velocity.Y = MathHelp.Round(vNew[i * 3 + 1], roundDecimalPlaces);
                bodies[i].AngularVelocity = MathHelp.Round(vNew[i * 3 + 2], roundDecimalPlaces);
            }
        }
            ...
            Matrix initialLambda, Matrix minLambda, Matrix maxLambda, Matrix lambda, Matrix vNew)
        {
            ...
            return PrintTwoColums(part1, part2);
        }
