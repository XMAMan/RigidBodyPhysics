    ...
    internal class AddRectangleFunction : DummyFunction, IFunction
    {
        ...
        public override void HandleMouseMove(MouseEventArgs e)
        {
            if (this.mode == Mode.CornerMode && this.shiftIsPressed && this.firstMouseClick != null)
            {
                ...
                Vec2D p1 = new Vec2D(this.firstMouseClick.Value.X, this.firstMouseClick.Value.Y);
                Vec2D p2 = new Vec2D(e.X, e.Y);

                Vec2D dir = (p2 - p1).SignZeroIsOne().Normalize();
                Vec2D p3 = p1 + dir * ((p2 - p1) * dir);
                this.currentMousePosition = new Point((int)p3.X, (int)p3.Y);
            }
            else
            {
        ...
        private RotatedRectangle GetRotatedRectangle(Point p1, Point p2)
        {
            ...
            {
                Center = center,
                Size = new Vec2D(p1ToP2.Length(), this.currentLineModeWidth),
                AngleInDegree = angle
            };
        }
