...
namespace PhysicEngine.UnitTests.CollisionResolution
{
    ...
    public class PositionCorrectionTests
    {
        ...
        {
            var result = SimulateSeveralTimeSteps.DoTest(TestData + "CubeStack.txt", TimeStepTickRate, true, 100, 500, new SimulateSeveralTimeSteps.ExtraSettings() { DoPositionCorrection = true });
        ...
        public void LongCubeStack_FallingDown_BecomeCalm()
        {
            for (int i = 0; i < 20; i++)
            {
                var result = SimulateSeveralTimeSteps.DoTest(TestData + "LongCubeStack.txt", TimeStepTickRate, true, 20, 500, new SimulateSeveralTimeSteps.ExtraSettings() { DoPositionCorrection = true });
        ...
        //Hinweis: Der Grund warum beim Box2DLite-Solver der Test rot wird ist, weil er pro Kontaktpunkt immer gleich Normal-Impuls+FrictionImpuls anwendet
        ...
        public void FunnelBalls_FallingDown_BecomeCalm()
        {
            ...
            maxCalm.Should().BeLessThanOrEqualTo(150); //Spätestens nach 150 TimeSteps soll jeder Körper in Ruhelage sein
        }
        ...
        {
            var result = SimulateSeveralTimeSteps.DoTest(TestData + "Pyramid.txt", TimeStepTickRate, true, 5, 60, new SimulateSeveralTimeSteps.ExtraSettings() { DoPositionCorrection = true });
