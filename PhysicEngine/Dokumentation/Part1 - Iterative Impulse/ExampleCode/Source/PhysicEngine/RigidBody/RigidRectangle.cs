...
namespace PhysicEngine.RigidBody
{
    public class RigidRectangle : IRigidBody, ICollidableRectangle
    {
        ...
        public RigidRectangle(Vector2D center, Vector2D size, float angle, float mass)
        {
            this.Center = center;
            this.Angle = angle;
        ...
        public RigidRectangle(RectangleExportData data)
            :this(data.Center, data.Size, data.AngleInDegree / 180 * (float)Math.PI, data.Mass)
        {
        ...
        public IExportShape GetExportData()
        {
            return new RectangleExportData()
            {
                ...
                Mass = (this.InverseMass == 0 ? float.MaxValue : 1.0f / this.InverseMass),
