...
namespace PhysicEngine.UnitTests.SoftConstraints
{
    ...
    {
        //Dieser Test zeigt, bei welchen Verfahren die Feder explodiert, wenn die Frequenz (und damit die Federsteifigkeit) zu hoch wird
        [Fact]
        public void UndampedStiffSpring_CompareSolvers()
        {
            float frequency = 6.4f; //Ab dieser Frequenz explodiert die Feder bei SemiImplicitEuler
            float m = 1;
            float xStart = 100;
            float h = 50;
            List<float[]> helper = new List<float[]>();
            helper.Add(UndampedSpringWithExplicitFormular(frequency, xStart, m, h));
            helper.Add(UndampedSpringWithExplicitEuler(frequency, xStart, m, h));
            helper.Add(UndampedSpringWithImplicitEuler(frequency, xStart, m, h));
            helper.Add(UndampedSpringWithSemiImplicitEuler(frequency, xStart, m, h));
            
            helper.Add(UndampedSpringWithSemiImplicitEulerAndSoftConstraint(0, frequency, xStart, m, h));
            helper.Add(UndampedSpringWithSemiImplicitEulerAndSoftConstraint(2, frequency, xStart, m, h));

            StringBuilder str = new StringBuilder();
            str.AppendLine("ExplicitFormular\tExplicitEuler\tImplicitEuler\tSemiImplicitEuler\tSemiImplicitEulerAndSoftConstraint_v2_x2\tSemiImplicitEulerAndSoftConstraint_x2_v2");
        ...
        //Vergleicht ein kleines Gewicht, was ungedämpft schwingt
        [Fact]
        ...
        public float[] UndampedSpringWithExplicitEuler(float frequency = 1, float xStart = 100, float m = 1, float h = 50)
        {
            ...
            for (float t = 0; t <= 3000; t += h)
            {
                float v2 = v - h * k / m * x; //Explicit-Euler
                float x2 = x + h * v;
        ...
        public float[] UndampedSpringWithImplicitEuler(float frequency = 1, float xStart = 100, float m = 1, float h = 50)
        {
            ...
            for (float t = 0; t <= 3000; t += h)
            {
                ...
                //v2 = v - h * k / m * x2;                      //Original-V2-Formel -> Funktioniert nur wenn die frequency nicht zu hoch ist
