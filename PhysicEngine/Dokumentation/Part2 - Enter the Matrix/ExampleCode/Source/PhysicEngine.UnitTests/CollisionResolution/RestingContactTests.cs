...
namespace PhysicEngine.UnitTests.CollisionResolution
{
    ...
    public class RestingContactTests
    {
        ...
        {
            int cube = 3;
            var result = SimulateSeveralTimeSteps.DoTest(TestData + "RestingCube.txt", TimeStepTickRate, true, 100, 900);
        ...
        {
            int cube = 1;
            var result = SimulateSeveralTimeSteps.DoTest(TestData + "CippingCube.txt", TimeStepTickRate, false, 100, 500);
        ...
        {
            var result = SimulateSeveralTimeSteps.DoTest(TestData + "Friction.txt", TimeStepTickRate, false, 100, 27);
            ...
            for (int i=1;i<result.BodyCount;i++)
            {
                ...
                v = v.GetRange(2, v.Count - 2); //Überspringe den ersten TimeStep da dort die Geschwindigkeit noch null ist und den zweiten, weil da die Geschwindigkeit noch so gering ist, dass der noramlisierte Länge anders ist, als vom ganzen Rest
        ...
        public void CubeOnTable_StartsWithMomentum_BecomesQuickResting()
        {
            ...
            //Würfel wird mit AllowedPenetration=2.0 unter Nutzung von PositionCorrection simuliert -> Hier kommt der Würfel zur Ruhe
            ...
            range.Should().Be(0); //Die Ausrichtung ändert sich nach 10 TimeSteps nicht mehr
        }
    }
}
