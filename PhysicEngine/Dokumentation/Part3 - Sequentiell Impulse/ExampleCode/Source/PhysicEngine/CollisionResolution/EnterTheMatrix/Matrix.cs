...
namespace PhysicEngine.CollisionResolution.EnterTheMatrix
{
    public class Matrix
    {
        ...
        public Matrix GetRange(int startX, int startY, int width, int height)
        {
            float[,] range = new float[width, height];
            for (int x = 0; x < range.GetLength(0); x++)
                for (int y = 0; y < range.GetLength(1); y++)
                    range[x, y] = this.m[startX + x, startY + y];

            return new Matrix(range);
		}
