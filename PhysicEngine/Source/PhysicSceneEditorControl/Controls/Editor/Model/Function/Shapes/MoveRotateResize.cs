...
namespace PhysicSceneEditorControl.Controls.Editor.Model.Function.Shapes
{
    internal abstract class MoveRotateResize : DummyFunction, IFunction
    {
        ...
        protected List<IEditorAxialFriction> axialFrictions = null;
        ...
        protected void UpdateJointsAndThrusters(IEditorShape selectedShape)
        {
            ...
            var axialFrictionsToUpdate = axialFrictions.Where(x => x.Body == selectedShape).ToList();
            axialFrictionsToUpdate.ForEach(x => x.UpdateAfterMovingBodys());
        }
