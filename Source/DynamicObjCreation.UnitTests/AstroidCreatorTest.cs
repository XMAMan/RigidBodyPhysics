...
namespace DynamicObjCreation.UnitTests
{
    ...
    public class AstroidCreatorTest
    {
        ...
        [Fact] //StaFact Wird beötigt, da das OpenTK-WinForm-Control ansonsten nur 333 Pixel breit/hoch anstatt 500 ist
        public void CreateMultipleAstroids()
        {
            int size = 50;
            var panel = new DrawingPanel.DrawingPanel(size * 10, size * 10);
            ...
            for (int x=0;x<10;x++)
                for (int y=0;y<10;y++)
                {
                    ...
                    //var points = AstroidCreator.CreateAstroidPolygon(rand, size / 2, 25).Select(x => (x + center)).ToList();
                    //panel.DrawPolygon(new Pen(Color.Black, 2), points);
                    //panel.DrawCircle(Pens.Blue, center, size / 2);
