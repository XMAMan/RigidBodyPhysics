...
namespace PhysicSceneEditorControl.Controls.Editor.Model
{
    ...
    {
        public static void DrawStringOnCircleBorder(this IDrawingPanel panel, string text, float textSize, Color color, Vec2D pos, Vec2D dir)
        {
            Vec2D textPos1 = GetTextPosition(panel, text, textSize, pos, dir);
            panel.DrawString(textPos1, color, textSize, text);
        }
        ...
        private static Vec2D GetTextPosition(IDrawingPanel panel, string text, float textSize, Vec2D pos, Vec2D dir)
        {
