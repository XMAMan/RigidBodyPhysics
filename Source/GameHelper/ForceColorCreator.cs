...
namespace GameHelper
{
    public static class ForceColorCreator
    {
        ...
        public static Vec3D GetForceColor(float force,float minForce, float maxForce)
        {
            ...
            if (f == 0) return new Vec3D(0, 1, 0);

            Vec3D color;
            if (f < 0)
            {
                var c1 = new Vec3D(0, 0, 1);  //Maximal Zug = Blau
                var c2 = new Vec3D(0, 1, 0);  //Keine Kraft = Grün
                f += 1;
                color = (1 - f) * c1 + f * c2;
            ...
            {
                var c1 = new Vec3D(0, 1, 0);  //Keine Kraft = Grün
                var c2 = new Vec3D(1, 0, 0);  //Maximal Druck = Rot
                color = (1 - f) * c1 + f * c2;
            }
