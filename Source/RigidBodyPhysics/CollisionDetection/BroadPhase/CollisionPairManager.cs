...
namespace RigidBodyPhysics.CollisionDetection.BroadPhase
{
    ...
    internal class CollisionPairManager<T> where T : class, ICollidable, IBoundingCircle
    {
        ...
        public List<CollidablePair<T>> GetPairListWithExternCollidable(T externCollidable)
        {
            List<CollidablePair<T>> list = new List<CollidablePair<T>>();
            for (int i=0; i < collidables.Count;i++)
            {
                var b1 = collidables[i];
                if (ShouldCollide(collidables[i], externCollidable, CollisionMatrix))
                    list.Add(new CollidablePair<T>(b1, externCollidable, i, -1));
            }
            return list;
        }
