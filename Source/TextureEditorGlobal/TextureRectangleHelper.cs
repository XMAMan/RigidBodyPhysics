...
namespace TextureEditorGlobal
{
    ...
    public static class TextureRectangleHelper
    {
        ...
        public static Vec2D[] GetTextureBorderPoints(Vec2D bodyCenter, Vec2D localAABBCenter, float width, float height, float angleInDegree, float deltaX, float deltaY, float deltaAngle)
        {
            ...
            var texCenter = new Vec2D(localCenter.X, localCenter.Y) + new Vec2D(deltaX, deltaY); //Richtungsvektor vom Masseschwerpunkt zum Textur-Rechteck-Zentrum

            //Schritt 2: An der texCenter-Position das TexturRechteck erstellen
            Vec2D[] local = new Vec2D[]
            {
                new Vec2D(texCenter.X - width / 2, texCenter.Y - height / 2),
                new Vec2D(texCenter.X + width / 2, texCenter.Y - height / 2),
                new Vec2D(texCenter.X + width / 2, texCenter.Y + height / 2),
                new Vec2D(texCenter.X - width / 2, texCenter.Y + height / 2)
            };
            ...
            var points = local.Select(x => Vec2D.RotatePointAroundPivotPoint(texCenter, x, deltaAngle)).ToList();
            points = points.Select(x => Vec2D.RotatePointAroundPivotPoint(bodyCenter, x, angleInDegree)).ToList();
